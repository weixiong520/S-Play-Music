<template>
  <div class="settings-page" @click="clickOutside">
    <div class="container">
      ...
      <div v-if="isElectron" class="item">
        <div class="left">
          <div class="title"> {{ $t('settings.cacheDirectory.text') }} </div>
          <div class="description">{{ cacheDirectoryPath }}</div>
        </div>
        <div class="right cache-directory-actions">
          <button @click="chooseCacheDirectory">
            {{ $t('settings.cacheDirectory.choose') }}
          </button>
          <button @click="resetCacheDirectoryPath">
            {{ $t('settings.cacheDirectory.reset') }}
          </button>
          <button @click="openCurrentCacheDirectory">
            {{ $t('settings.cacheDirectory.open') }}
          </button>
        </div>
      </div>
      <div v-if="isElectron" class="item">
  ...
  methods: {
    ...
    countDBSize() {
      countDBSize().then(data => {
        ...
        if (data.path) this.cacheDirectoryPath = data.path;
      });
    },
    clearCache() {
      clearDB().then(() => {
    ...
    refreshCacheInfo() {
      getCacheInfo().then(data => {
        if (!data) return;
        this.cacheDirectoryPath = data.path;
        this.tracksCache.size = bytesToSize(data.bytes);
        this.tracksCache.length = data.length;
      });
    },
    chooseCacheDirectory() {
      selectCacheDirectory().then(data => {
        if (!data || data.canceled) return;
        this.cacheDirectoryPath = data.path;
        this.$store.commit('updateSettings', {
          key: 'cacheDirectoryPath',
          value: data.isDefault ? '' : data.path,
        });
        this.autoMigrateOldCache().then(migrated => {
          this.countDBSize();
          this.showToast(
            `已更新缓存目录，并迁移 ${
              data.fileMigrated || 0
            } 个文件缓存、${migrated} 首旧缓存歌曲`
          );
        });
      });
    },
    resetCacheDirectoryPath() {
      resetCacheDirectory().then(data => {
        if (!data) return;
        this.cacheDirectoryPath = data.path;
        this.$store.commit('updateSettings', {
          key: 'cacheDirectoryPath',
    ...
    openCurrentCacheDirectory() {
      openCacheDirectory().then(result => {
        if (!result?.ok) {
          this.showToast(result?.errorMessage || '无法打开缓存目录');
        }
      });
    },
    autoMigrateOldCache() {
      return migrateIndexedDBCacheToFile()
        .then(result => result.migrated)
        .catch(error => {
          console.debug('[settings.vue] migrate old cache failed', error);
          this.showToast('缓存目录已更新，旧缓存迁移失败');
          return 0;
        });
    },
    lastfmConnect() {
