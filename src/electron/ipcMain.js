...
export function initIpcMain(win, store, trayEventEmitter) {
  ...
  ipcMain.handle('cache:getInfo', () => getCacheInfo(store));
  ipcMain.handle('cache:selectDirectory', () =>
    selectCacheDirectory(win, store)
  );
  ipcMain.handle('cache:resetDirectory', () => resetCacheDirectory(store));
  ipcMain.handle('cache:openDirectory', () => openCacheDirectory(store));
  ipcMain.handle('cache:getTrackSource', (_, id) => getTrackSource(store, id));
  ipcMain.handle('cache:saveTrackSource', (_, track) =>
    saveTrackSource(store, track)
  );
  ipcMain.handle('cache:clear', () => clearCache(store));
